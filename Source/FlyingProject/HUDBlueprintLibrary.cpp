...
void UHUDBlueprintLibrary::GetLockableLocation(UObject * WorldContextObject, AActor* Target, bool & bIsALockable, FVector & OutLocation)
{
	ILockable* lockableTarget = Cast<ILockable>(Target);
	if (lockableTarget != nullptr)
	{
		bIsALockable = true;
		OutLocation = lockableTarget->Execute_GetLockableLocation(Target);
	}
	else
	{
		OutLocation = FVector();
...
/*USceneComponent* UHUDBlueprintLibrary::GetLockableSceneComponent(UObject * WorldContextObject, AActor * Target, bool & bIsALockable)
{
	ILockable* lockableTarget = Cast<ILockable>(Target);
	if (lockableTarget != nullptr)
	{
		bIsALockable = true;
		return lockableTarget->Execute_GetSceneComponent(Target);
	}
	else
	{
		bIsALockable = false;
